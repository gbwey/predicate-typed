...
unnamedTests = [
  ...
  , (@?=) (unsafeRefined @((Len >> Same 4) && Luhn Id) [1,2,3,0]) $$(refinedTH [1,2,3,0])
  , (@?=) (unsafeRefined @(Not ((Len >> Same 4) && Luhn Id)) [1,2,3,1]) $$(refinedTH [1,2,3,1])
...
type Ip6B = Skip (Guard "count is bad" (Len >> Between 0 8))
...
type Ip6B' = Skip (Guard "count is bad" (Len >> Same 8))
         |> Guard "out of bounds" (All (Between 0 65535) Id)

type Ip4A = Map (ReadBaseInt 10 Id) (Resplit "\\." Id)
type Ip4B = Skip (Guard "expected 4 numbers" (Len >> Same 4))
