...
allTests =
  ...
  , expectPE (PresentT 55) $ pl @(Map (Wrap (SG.Sum _) Id) Id >> MConcat Id >> Unwrap) [1..10]
...
-- have to check the length of the match vs input to see that are the same
  ...
  , expectPE (PresentT [13,2,1999]) $ pl @(Rescan DdmmyyyyRE >> OneP >> Map (ReadP Int Id) Snd) "13-02-1999"
  , expectPE (PresentT [3,2,1999]) $ pl @(Rescan DdmmyyyyRE >> OneP >> Map (ReadP Int Id) Snd >| Ddmmyyyyop) "03-02-1999"
  , expectPE (FailT "month 13 is out of range") $ pl @(Rescan DdmmyyyyRE >> OneP >> Map (ReadP Int Id) Snd >| Ddmmyyyyop) "12-13-1999"
